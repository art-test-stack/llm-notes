#!/usr/bin/env python3
"""Public-content schema and privacy checks without committed private names."""
...
# These checks detect generic data-leak shapes in publication content.
# Context-specific names belong in the private exporter or optional secret.
...
def _optional_private_patterns() -> list[re.Pattern[str]]:
    """Load an optional context-specific denylist from an Actions secret.

    PUBLICATION_DENYLIST_B64 is base64-encoded UTF-8 JSON containing a list of
    regular-expression strings. Patterns and matches are never printed.
    """
...
def scan_repository_with_optional_policy(root: Path) -> None:
    """Scan all repository text only with patterns held outside the repository."""
    private_patterns = _optional_private_patterns()
    ...
    for path in sorted(root.rglob("*")):
        if (
            ...
            or path.suffix.lower() in {".pyc", ".png", ".ico"}
        ):
            continue
        try:
        ...
        for index, pattern in enumerate(private_patterns):
            if pattern.search(text):
                raise ValueError(
                    f"Restricted publication pattern #{index + 1} matched in {path.relative_to(root)}"
                )
